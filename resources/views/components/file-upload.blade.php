...
<div x-data="{
    ...
    @endif
    @if($current)
        <p class="mb-1.5 text-xs text-on-surface-muted">Fichier actuel : {{ $current }}</p>
    @endif

    {{-- Image preview --}}
    <div x-show="previewUrl" x-cloak class="mb-2 relative inline-block">
        <img :src="previewUrl" alt="Apercu" class="h-28 w-auto rounded-lg border border-theme object-cover shadow-sm">
    ...
    <label :for="'{{ $inputId }}'"
        ...
        <p class="text-xs text-on-surface-muted" x-show="!fileName"><span class="font-semibold text-brand-600 dark:text-brand-400">Cliquer pour telecharger</span> ou glisser-deposer</p>
        <p class="text-xs font-medium text-brand-700 dark:text-brand-300" x-show="fileName" x-text="fileName" x-cloak></p>
        <p class="mt-1 text-xs text-on-surface-faint">{{ $hint ?? 'JPG, PNG ou PDF (max 5 Mo)' }}</p>
    </label>

    {{-- Non-image file name display --}}
    <div x-show="fileName && !previewUrl" x-cloak class="mt-2 flex items-center gap-2 text-sm text-on-surface-secondary">
