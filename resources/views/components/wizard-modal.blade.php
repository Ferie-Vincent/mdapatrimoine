...
>
    ...
    <div x-show="show"
        ...
        {{-- Header --}}
        <div class="px-6 py-4 border-b border-theme shrink-0">
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="close()" class="text-on-surface-faint hover:text-on-surface-secondary transition">
                ...
                @foreach($steps as $i => $stepLabel)
                    <div class="flex items-center gap-2 {{ $i < count($steps) - 1 ? 'flex-1' : '' }}">
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <span :class="currentStep >= {{ $i }} ? 'text-on-surface' : 'text-on-surface-faint'"
                        ...
                        @if($i < count($steps) - 1)
                            <div class="flex-1 h-0.5 rounded-full transition-colors"
                                 :class="currentStep > {{ $i }} ? 'bg-brand-600' : 'bg-theme'"></div>
                        @endif
                    </div>
                @endforeach
            </div>
        ...
        <template x-if="errors._general">
            <div class="mx-6 mt-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/30 text-red-700 dark:text-red-300 rounded-lg px-4 py-3 text-sm">
        ...
        <form action="{{ $action }}" @submit="submit($event)" {{ $hasFiles ? 'enctype=multipart/form-data' : '' }} class="flex flex-col flex-1 overflow-hidden">
            ...
            {{-- Footer --}}
            <div class="flex items-center justify-between px-6 py-4 border-t border-theme shrink-0 bg-surface-hover">
                ...
                <div class="flex items-center gap-3">
                    <button type="button" @click="close()" class="inline-flex items-center justify-center rounded-lg border border-theme px-5 py-2.5 text-sm font-medium text-on-surface-secondary bg-surface hover:bg-surface-hover transition">
                        Annuler
                    </button>
