...
@section('content')
    ...
    <x-filters action="{{ route('service-providers.index') }}">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label for="category" class="block text-xs font-semibold text-on-surface-muted/80 uppercase tracking-wider mb-1">Catégorie</label>
                <select name="category" id="category" class="block w-full rounded-xl border-theme bg-surface-hover/70 text-sm text-on-surface hover:border-brand-200 dark:border-gray-600 focus:bg-surface focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 transition-all py-2.5">
            ...
            <div>
                <label for="is_active" class="block text-xs font-semibold text-on-surface-muted/80 uppercase tracking-wider mb-1">Statut</label>
                <select name="is_active" id="is_active" class="block w-full rounded-xl border-theme bg-surface-hover/70 text-sm text-on-surface hover:border-brand-200 dark:border-gray-600 focus:bg-surface focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 transition-all py-2.5">
                ...
                <table id="dataTable" class="min-w-full divide-y divide-theme-subtle">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Categorie</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Specialite</th>
                            ...
                            <tr class="hover:bg-surface-hover/50 transition">
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-on-surface">{{ $provider->name }}</div>
                                            @if($provider->company)
                                                <div class="text-xs text-on-surface-faint">{{ $provider->company }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $provider->specialty ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $provider->phone ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $provider->email ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap">
    ...
    @if(isset($expiringContracts) && $expiringContracts->count())
        <div class="mt-6 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/30 p-4 flex items-start gap-3">
            <svg class="w-5 h-5 text-amber-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/>
            </svg>
            <div>
                <p class="text-sm font-semibold text-amber-800 dark:text-amber-200">{{ $expiringContracts->count() }} contrat(s) expirant dans les 30 prochains jours</p>
                <ul class="mt-1 text-sm text-amber-700 dark:text-amber-300">
    ...
    <div class="flex items-center justify-between mt-8 mb-4">
        <h2 class="text-lg font-semibold text-on-surface">Contrats fournisseurs</h2>
        @can('create', App\Models\ServiceProvider::class)
    ...
    <div class="bg-surface rounded-2xl border border-theme-subtle shadow-sm overflow-hidden">
        @if(isset($contracts) && $contracts->count())
            <div class="overflow-x-auto">
                <table id="contractsTable" class="min-w-full divide-y divide-theme-subtle">
                    <thead>
                        <tr>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Titre</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Prestataire</th>
                    ...
                    <tbody class="divide-y divide-theme-subtle">
                        @foreach($contracts as $contract)
                            ...
                            @endphp
                            <tr class="hover:bg-surface-hover/50 transition {{ $isExpiring ? 'bg-amber-50/30 dark:bg-amber-950/30' : '' }}">
                                <td class="px-6 py-5 whitespace-nowrap">
                                    ...
                                    @if($contract->description)
                                        <div class="text-xs text-on-surface-faint truncate max-w-xs">{{ Str::limit($contract->description, 60) }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $contract->serviceProvider->name }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm font-medium text-on-surface text-right">{{ number_format((float) $contract->amount, 0, ',', ' ') }} F</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $contract->start_date->format('d/m/Y') }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $contract->end_date ? $contract->end_date->format('d/m/Y') : '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                    @php
                                    ...
                                    <div class="inline-flex items-center gap-2">
                                        @can('create', App\Models\ServiceProvider::class)
                                            <button type="button" data-open-modal="edit-contract-{{ $contract->id }}"
                                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-950/30 border border-brand-200 dark:border-gray-600 rounded-lg hover:bg-brand-100 dark:hover:bg-brand-800/40 dark:bg-brand-900/40 transition"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Modifier</button>
                                            <button type="button" x-data="deleteAction" @click="confirmDelete('{{ route('provider-contracts.destroy', $contract) }}', $el.closest('tr'), 'ce contrat')" :disabled="loading" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/30 rounded-lg hover:bg-red-100 dark:hover:bg-red-800/40 dark:bg-red-900/40 transition disabled:opacity-50"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>Supprimer</button>
                                        @endcan
                                    </div>
                                </td>
                            </tr>
