...
@section('content')
    ...
    <x-filters action="{{ route('users.index') }}">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <label for="role" class="block text-xs font-semibold text-on-surface-muted/80 uppercase tracking-wider mb-1">Rôle</label>
                <select name="role" id="role" class="block w-full rounded-xl border-theme bg-surface-hover/70 text-sm text-on-surface hover:border-brand-200 dark:border-gray-600 focus:bg-surface focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 transition-all py-2.5">
    ...
    <div class="bg-surface rounded-2xl border border-theme-subtle shadow-sm overflow-hidden mt-6">
        @if($users->count())
            <table id="dataTable" class="min-w-full">
                <thead class="">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Role</th>
                        <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">SCIs</th>
                        <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Actif</th>
                        ...
                        <tr class="hover:bg-surface-hover/50 transition">
                            <td class="px-6 py-5 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-on-surface">{{ $user->name }}</div>
                                        <div class="text-xs text-on-surface-faint">{{ $user->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-5 whitespace-nowrap">
                            ...
                            <td class="px-6 py-5 text-sm text-on-surface-secondary max-w-xs truncate">{{ $user->scis->pluck('name')->join(', ') ?: '-' }}</td>
                            <td class="px-6 py-5 whitespace-nowrap">
            ...
            <div class="px-6 py-4 border-t border-theme-subtle">{{ $users->links() }}</div>
        @else
