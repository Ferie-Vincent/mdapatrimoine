...
@section('content')
    ...
    <div class="flex items-center gap-1 mb-6 border-b border-theme">
        <a href="{{ route('leases.index', array_merge(request()->except('status', 'page'), [])) }}"
           class="px-4 py-2.5 text-sm font-medium border-b-2 transition {{ !$isArchive ? 'border-brand-600 dark:border-brand-400 text-brand-600 dark:text-brand-400' : 'border-transparent text-on-surface-muted hover:text-on-surface-secondary' }}">
            Baux en cours
        </a>
    ...
    <div class="bg-surface rounded-2xl border border-theme-subtle shadow-sm overflow-hidden mt-6">
        @if($leases->count())
                ...
                <table id="dataTable" class="min-w-full divide-y divide-theme-subtle">
                    <thead class="">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Frais agence</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Debut</th>
                            ...
                            <tr class="hover:bg-surface-hover/50 transition">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <span class="text-sm font-semibold text-on-surface">{{ $lease->property->reference ?? '-' }}</span>
                                                @if($lease->isMeuble())
                                                    <span class="inline-flex items-center px-1.5 py-0.5 text-[10px] font-semibold rounded bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300">Meuble</span>
                                                @endif
                                            </div>
                                            <div class="text-xs text-on-surface-faint">{{ $lease->property->address ?? '' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm font-medium text-on-surface">{{ $lease->tenant->full_name ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm font-semibold text-on-surface text-right">
                                    @if($lease->isMeuble())
                                        {{ number_format($lease->total_amount, 0, ',', ' ') }} F
                                        <div class="text-[10px] font-normal text-on-surface-faint">{{ $lease->nights_count }} nuits</div>
                                    @else
                                        {{ number_format($lease->rent_amount, 0, ',', ' ') }} F
                                    @endif
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary text-right">{{ $lease->isMeuble() ? '-' : number_format($lease->charges_amount, 0, ',', ' ') . ' F' }}</td>
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-right text-sm">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('leases.show', $lease) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-on-surface-secondary bg-surface-hover border border-theme rounded-lg hover:bg-surface-alt transition"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>Voir</a>
                                        @if(!in_array($lease->status, ['resilie', 'termine']))
                                            @can('update', $lease)
                                                <button type="button" data-open-modal="edit-lease-{{ $lease->id }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-950/30 border border-brand-200 dark:border-gray-600 rounded-lg hover:bg-brand-100 dark:hover:bg-brand-800/40 dark:bg-brand-900/40 transition"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Modifier</button>
                                            @endcan
                                        @endif
                                    </div>
                                </td>
            ...
            <div class="px-6 py-4 border-t border-theme-subtle">{{ $leases->withQueryString()->links() }}</div>
        @else
    ...
    @can('create', App\Models\Lease::class)
        <x-wizard-modal name="create-lease" title="Nouveau bail" :action="route('leases.store')" :hasFiles="true"
            ...
            <div x-data="{
                ...
                if (isMeuble && checkIn && checkOut) {
                    const ci = new Date(checkIn + 'T00:00:00');
                    const co = new Date(checkOut + 'T00:00:00');
                    mNights = Math.max(0, Math.round((co - ci) / 86400000));
                    mSubtotal = dailyRate * mNights;
                    mDiscountRate = mNights >= 21 ? 15 : (mNights >= 14 ? 10 : (mNights >= 7 ? 5 : 0));
                    mDiscountAmount = Math.round(mSubtotal * mDiscountRate / 100);
                    mTotalAmount = mSubtotal - mDiscountAmount;
                } else {
                    mNights = 0; mSubtotal = 0; mDiscountRate = 0; mDiscountAmount = 0; mTotalAmount = 0;
                }
            ">
            ...
            <div x-show="currentStep === 0" data-step="0">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        @else
                            <select name="sci_id" required class="mt-1.5 h-11 w-full appearance-none rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden" onchange="window._leaseFilterSelects(this.value)">
                    ...
                    <div>
                        ...
                        <select name="property_id" id="create-lease-property" required
                                @change="const opt = $event.target.options[$event.target.selectedIndex]; isMeuble = (opt.dataset.category === 'meuble'); dailyRate = parseFloat(opt.dataset.dailyRate) || 0; checkIn = ''; checkOut = '';"
                            ...
                            @foreach($properties ?? [] as $prop)
                                <option value="{{ $prop->id }}" data-sci="{{ $prop->sci_id }}" data-category="{{ $prop->rental_category ?? 'non_meuble' }}" data-daily-rate="{{ $prop->daily_rate ?? 0 }}">{{ $prop->reference }} - {{ $prop->address }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="tenant_id" id="create-lease-tenant" required class="mt-1.5 h-11 w-full appearance-none rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                            <option value="">Selectionner</option>
                ...
                {{-- Meuble indicator --}}
                <div x-show="isMeuble" x-transition class="mt-4 flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/30 rounded-lg">
                    ...
                    <span class="text-sm text-amber-700 dark:text-amber-300">Ce bien est <strong>meuble</strong>. Le formulaire sera adapte pour une location courte duree.</span>
                </div>
            </div>

            {{-- Step 2: Termes du bail --}}
            <div x-show="currentStep === 1" data-step="1">
                {{-- Mode classique (non meuble) --}}
                <div x-show="!isMeuble" x-data="leaseDates()" x-effect="computeDuration()">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-on-surface-secondary">Date de debut <span class="text-red-500">*</span></label>
                            <input type="date" name="start_date" x-model="startDate" @change="onStartChange()" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" x-model="endDate" :min="startDate" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                        ...
                        <div>
                            ...
                            <x-money-input name="rent_amount" />
                    ...
                    <div class="mt-5 pt-4 border-t border-dashed border-theme"
                         x-data="{ rentRaw: 0 }"
                        ...
                        <p class="text-xs font-semibold text-on-surface-muted uppercase tracking-wider mb-3">Montants d'entree (calcul automatique)</p>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-on-surface-muted">Caution (2 mois)</label>
                                <div class="mt-1 h-10 w-full rounded-lg border border-theme bg-surface-hover px-4 py-2 text-sm text-on-surface-secondary flex items-center justify-between">
                                    <span x-text="(rentRaw * 2).toLocaleString('fr-FR')">0</span>
                                    <span class="text-xs font-medium text-on-surface-faint">FCFA</span>
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-on-surface-muted">Avances loyers (2 mois)</label>
                                <div class="mt-1 h-10 w-full rounded-lg border border-theme bg-surface-hover px-4 py-2 text-sm text-on-surface-secondary flex items-center justify-between">
                                    <span x-text="(rentRaw * 2).toLocaleString('fr-FR')">0</span>
                                    <span class="text-xs font-medium text-on-surface-faint">FCFA</span>
                                </div>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-on-surface-muted">Frais d'agence (1 mois)</label>
                                <div class="mt-1 h-10 w-full rounded-lg border border-theme bg-surface-hover px-4 py-2 text-sm text-on-surface-secondary flex items-center justify-between">
                ...
                <div x-show="isMeuble">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="date" name="check_in_date" x-model="checkIn" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                            <template x-if="errors.check_in_date"><p class="mt-1 text-sm text-red-600" x-text="errors.check_in_date[0]"></p></template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-on-surface-secondary">Date de check-out <span class="text-red-500">*</span></label>
                            <input type="date" name="check_out_date" x-model="checkOut" :min="checkIn" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                            <template x-if="errors.check_out_date"><p class="mt-1 text-sm text-red-600" x-text="errors.check_out_date[0]"></p></template>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-on-surface-secondary">Nombre de nuits</label>
                            <input type="number" :value="mNights" readonly class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-surface-alt px-4 py-2.5 text-sm text-on-surface cursor-not-allowed">
                        </div>
                    </div>

                    {{-- Hidden input for daily_rate --}}
                    <input type="hidden" name="daily_rate" :value="dailyRate">

                    {{-- Recap card --}}
                    <div x-show="mNights > 0" x-transition class="mt-5 p-4 bg-gradient-to-br from-surface-hover to-surface border border-theme rounded-xl">
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            <span class="px-2 py-1 text-xs rounded-full" :class="mDiscountRate === 0 ? 'bg-theme text-on-surface-secondary font-semibold' : 'bg-surface-alt text-on-surface-faint'">1-6 nuits : 0%</span>
                            <span class="px-2 py-1 text-xs rounded-full" :class="mDiscountRate === 5 ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-semibold' : 'bg-surface-alt text-on-surface-faint'">&ge; 7 nuits : -5%</span>
                            <span class="px-2 py-1 text-xs rounded-full" :class="mDiscountRate === 10 ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 font-semibold' : 'bg-surface-alt text-on-surface-faint'">&ge; 14 nuits : -10%</span>
                            <span class="px-2 py-1 text-xs rounded-full" :class="mDiscountRate === 15 ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 font-semibold' : 'bg-surface-alt text-on-surface-faint'">&ge; 21 nuits : -15%</span>
                        </div>

                        <div class="space-y-2">
                            <div class="flex justify-between text-sm">
                                <span class="text-on-surface-secondary">Tarif / 24h</span>
                                <span class="font-medium" x-text="dailyRate.toLocaleString('fr-FR') + ' F'"></span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-on-surface-secondary">Sous-total (<span x-text="mNights"></span> nuits)</span>
                                <span class="font-medium" x-text="mSubtotal.toLocaleString('fr-FR') + ' F'"></span>
                            </div>
                            <div x-show="mDiscountRate > 0" class="flex justify-between text-sm text-green-600">
                                <span>Remise (<span x-text="mDiscountRate"></span>%)</span>
                                <span class="font-medium">- <span x-text="mDiscountAmount.toLocaleString('fr-FR')"></span> F</span>
                            </div>
                            <div class="flex justify-between text-base font-bold pt-2 border-t border-theme">
                                <span class="text-on-surface">Total a payer</span>
                                <span class="text-brand-600 dark:text-brand-400" x-text="mTotalAmount.toLocaleString('fr-FR') + ' F'"></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="currentStep === 2" data-step="2">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div x-show="!isMeuble">
                        ...
                        <input type="number" name="penalty_rate" value="0" min="0" max="100" step="0.01" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                    </div>
                    <div x-show="!isMeuble">
                        <label class="block text-sm font-medium text-on-surface-secondary">Delai avant penalite (jours)</label>
                        <input type="number" name="penalty_delay_days" value="0" min="0" class="mt-1.5 h-11 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden">
                    </div>
                </div>
            </div>
            ...
            <div x-show="currentStep === 3" data-step="3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-file-upload-multiple name="entry_inspection" label="Etat des lieux d'entree" accept=".jpg,.jpeg,.png,.pdf" hint="PDF, JPG ou PNG (max 10 Mo chacun)" />
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-on-surface-secondary">Notes</label>
                        <textarea name="notes" rows="4" placeholder="Notes supplementaires..." class="mt-1.5 w-full rounded-lg border border-theme bg-transparent px-4 py-2.5 text-sm text-on-surface focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden"></textarea>
                    </div>
                </div>
            </div>

            </div> {{-- End meuble state wrapper --}}
        </x-wizard-modal>
    @endcan
        ...
        @can('update', $lease)
            @if($lease->isMeuble())
                ...
                <x-wizard-modal name="edit-lease-{{ $lease->id }}" title="Modifier le sejour #{{ $lease->id }}" :action="route('leases.update', $lease)" method="PUT" :hasFiles="true"
                    :steps="['Parties', 'Sejour', 'Conditions', 'Documents', 'Notes']" icon='<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>' iconColor="text-purple-500">
                    ...
                    <div x-show="currentStep === 1" data-step="1">
                        ...
                        <div x-show="editNights > 0" x-transition class="mt-5 p-4 bg-gradient-to-br from-surface-hover to-surface border border-theme rounded-xl">
                            ...
                            <div class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    <span class="text-on-surface-secondary">Tarif / 24h</span>
                                    <span class="font-medium" x-text="editDailyRate.toLocaleString('fr-FR') + ' F'"></span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-on-surface-secondary">Sous-total (<span x-text="editNights"></span> nuits)</span>
                                    <span class="font-medium" x-text="editSubtotal.toLocaleString('fr-FR') + ' F'"></span>
                                </div>
                                <div x-show="editDiscountRate > 0" class="flex justify-between text-sm text-green-600">
                                    <span>Remise (<span x-text="editDiscountRate"></span>%)</span>
                                    <span class="font-medium">- <span x-text="editDiscountAmount.toLocaleString('fr-FR')"></span> F</span>
                                </div>
                                <div class="flex justify-between text-base font-bold pt-2 border-t border-theme">
                                    <span class="text-on-surface">Total a payer</span>
                                    <span class="text-brand-600 dark:text-brand-400" x-text="editTotalAmount.toLocaleString('fr-FR') + ' F'"></span>
                                </div>
                            </div>
                        </div>
                    </div>
