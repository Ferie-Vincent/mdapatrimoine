...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-on-surface">Vue d'ensemble</h2>
    </div>
    ...
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-on-surface">Mois en cours ({{ $monthStats['label'] }})</h2>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-on-surface-muted">Taux d'impaye</p>
                    <p class="text-2xl font-bold text-on-surface mt-1">{{ $unpaidTrendLast }}%</p>
        ...
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-on-surface-muted">Croissance locative</p>
                    <p class="text-2xl font-bold text-on-surface mt-1">{{ $chartData['active_leases'] }} baux</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        {{-- Area chart (2/3) --}}
        <div class="lg:col-span-2 bg-surface rounded-2xl border border-theme-subtle p-5">
            <h3 class="text-base font-semibold text-on-surface mb-4">Encaissements mensuels</h3>
            <div id="chart-monthly" class="w-full" style="min-height: 320px;"></div>
        </div>

        {{-- Donut chart (1/3) --}}
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <h3 class="text-base font-semibold text-on-surface mb-4">Occupation des biens</h3>
            <div id="chart-occupation" class="w-full" style="min-height: 320px;"></div>
        </div>
    </div>

    {{-- Map: Localisation des biens --}}
    <div class="bg-surface rounded-2xl border border-theme-subtle p-5 mb-6">
        <h3 class="text-base font-semibold text-on-surface mb-4">Localisation des biens</h3>
        <div id="map-properties" style="height:400px; position:relative; z-index:0;" class="rounded-xl"></div>
        @if($mapProperties->isEmpty())
            <p class="text-sm text-on-surface-faint text-center mt-3">Aucun bien avec coordonnees GPS. Editez un bien pour ajouter sa latitude / longitude.</p>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <h3 class="text-base font-semibold text-on-surface mb-4">Taux de recouvrement (12 mois)</h3>
            <div id="chart-recovery-rate" class="w-full" style="min-height: 320px;"></div>
        </div>
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <h3 class="text-base font-semibold text-on-surface mb-4">Evolution des revenus</h3>
    ...
    <div class="bg-surface rounded-2xl border border-theme-subtle p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-base font-semibold text-on-surface">Projection de revenus</h3>
            <span class="text-xs font-medium text-on-surface-faint bg-surface-alt rounded-full px-3 py-1">6 mois historique + 6 mois projection</span>
        </div>
    ...
    @if(($expiringContractsCount ?? 0) > 0)
        <div class="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4 mb-6 flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {{-- Overdue table (2/3) --}}
        <div class="lg:col-span-2 bg-surface rounded-2xl border border-theme-subtle overflow-hidden">
            {{-- Header --}}
            <div class="flex items-center justify-between px-6 py-4 border-b border-theme-subtle">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-base font-semibold text-on-surface">Echeances en retard</h3>
                        <p class="text-xs text-on-surface-faint mt-0.5">{{ $overdueMonthlies->count() }} echeance(s) en attente de paiement</p>
                    </div>
                </div>
                @if($overdueMonthlies->count())
                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-accent-red-50 dark:bg-accent-red-950/30 text-accent-red-400">
            ...
            @if($overdueMonthlies->count())
                <div class="divide-y divide-theme-subtle">
                    @foreach($overdueMonthlies->take(8) as $monthly)
                        <div class="flex items-center gap-4 px-6 py-4 hover:bg-surface-hover/60 transition group">
                            {{-- Avatar --}}
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-on-surface truncate">{{ $monthly->lease->tenant->full_name ?? '-' }}</p>
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="inline-flex items-center gap-1 text-xs text-on-surface-faint">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        {{ $monthly->month }}
                                    </span>
                                    @if($monthly->lease->property ?? null)
                                        <span class="text-on-surface-faint">&middot;</span>
                                        <span class="text-xs text-on-surface-faint truncate">{{ $monthly->lease->property->reference ?? '' }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="text-right shrink-0">
                                ...
                                <p class="text-[10px] text-on-surface-faint mt-0.5">reste a payer</p>
                            </div>

                            {{-- Actions --}}
                            <div class="flex items-center gap-2 shrink-0 ml-2">
                                <a href="{{ route('monthlies.show', $monthly) }}"
                                   class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-on-surface-muted bg-surface-hover rounded-lg hover:bg-surface-alt hover:text-on-surface-secondary transition">
        ...
        {{-- Recent activity (1/3) --}}
        <div class="bg-surface rounded-2xl border border-theme-subtle p-5">
            <h3 class="text-base font-semibold text-on-surface mb-4">Activite recente</h3>
            <div class="space-y-4">
                @foreach(($chartData['recent_payments'] ?? []) as $payment)
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-full bg-accent-green-50 dark:bg-accent-green-950/30 flex items-center justify-center shrink-0 mt-0.5">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-on-surface">Paiement de {{ number_format($payment['amount'], 0, ',', ' ') }} F</p>
                            <p class="text-xs text-on-surface-muted">{{ $payment['tenant'] }} &middot; {{ $payment['paid_at'] }}</p>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach(($chartData['recent_leases'] ?? []) as $lease)
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-full bg-brand-50 dark:bg-brand-950/30 flex items-center justify-center shrink-0 mt-0.5">
                            <svg class="w-4 h-4 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                        </div>
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-on-surface">Bail {{ $lease['status'] }}</p>
                            <p class="text-xs text-on-surface-muted">{{ $lease['tenant'] }} &middot; {{ $lease['property'] }} &middot; {{ $lease['date'] }}</p>
                        </div>
                    </div>
                @endforeach

                @if(empty($chartData['recent_payments']) && empty($chartData['recent_leases']))
                    <p class="text-sm text-on-surface-faint text-center py-4">Aucune activite recente</p>
                @endif
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getChartTheme() {
        ...
        return {
            ...
            tooltip: s.getPropertyValue('--color-surface').trim() || '#ffffff',
            tooltipText: s.getPropertyValue('--color-on-surface').trim() || '#111827',
    ...
    const cMonthly = new ApexCharts(document.querySelector('#chart-monthly'), {
        chart: { type: 'area', height: 320, toolbar: { show: false }, fontFamily: 'Nunito, sans-serif', background: 'transparent' },
        series: [
    ...
    const cOccupation = new ApexCharts(document.querySelector('#chart-occupation'), {
        chart: { type: 'donut', height: 320, fontFamily: 'Nunito, sans-serif', background: 'transparent' },
        ...
        legend: { position: 'bottom', fontSize: '12px', labels: { colors: ct.label } },
    ...
    cOccupation.render();
    allCharts.push(cOccupation);

    // ── Line Chart: Taux de recouvrement 12 mois ──
    const cRecovery = new ApexCharts(document.querySelector('#chart-recovery-rate'), {
        chart: { type: 'line', height: 320, toolbar: { show: false }, fontFamily: 'Nunito, sans-serif', background: 'transparent' },
        series: [{
        ...
        yaxis: {
            max: 100, min: 0,
            labels: { formatter: v => v.toFixed(0) + '%', style: { fontSize: '11px', colors: ct.label } },
        },
        colors: ['#D4812A'],
    ...
    const cRevenue = new ApexCharts(document.querySelector('#chart-revenue-evolution'), {
        chart: { type: 'bar', height: 320, toolbar: { show: false }, fontFamily: 'Nunito, sans-serif', background: 'transparent' },
        series: [
            { name: 'Attendu', data: monthlyTrend.map(m => m.expected) },
            { name: 'Encaisse', data: monthlyTrend.map(m => m.collected) },
        ],
    ...
    const cProjection = new ApexCharts(document.querySelector('#chart-projection'), {
        chart: { type: 'line', height: 340, toolbar: { show: false }, fontFamily: 'Nunito, sans-serif', background: 'transparent' },
        series: [
        ...
        dataLabels: { enabled: false },
        tooltip: { y: { formatter: v => v !== null ? new Intl.NumberFormat('fr-FR').format(v) + ' F' : '-' }, theme: ct.tooltipTheme },
        grid: { borderColor: ct.grid, strokeDashArray: 4 },
        legend: { position: 'top', horizontalAlign: 'right', fontSize: '12px', labels: { colors: ct.label } },
    });
    cProjection.render();
    allCharts.push(cProjection);
