...
@section('content')
    ...
    <x-filters action="{{ route('properties.index') }}">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label for="type" class="block text-xs font-semibold text-on-surface-muted/80 uppercase tracking-wider mb-1">Type de bien</label>
        ...
        @if($properties->count())
            <div class="overflow-x-auto">
                <table id="dataTable" class="min-w-full divide-y divide-theme-subtle">
                    <thead class="">
                        <tr>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Reference</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">N° Appartement</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Adresse</th>
                            <th class="px-6 py-3.5 text-left text-sm font-medium text-on-surface-faint">Ville</th>
                            ...
                            <tr class="hover:bg-surface-hover/50 transition">
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-semibold text-on-surface">{{ $property->reference }}</div>
                                            <div class="text-xs text-on-surface-faint">{{ ucfirst($property->type) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $property->numero_porte ?? '-' }}</td>
                                <td class="px-6 py-5 text-sm text-on-surface-secondary max-w-xs truncate">{{ $property->address }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $property->city ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $property->sci->name ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap">
