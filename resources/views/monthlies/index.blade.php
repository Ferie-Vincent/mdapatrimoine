                ...
                <table id="dataTable" class="min-w-full">
                    <thead class="">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Loyer dû</th>
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Pénalités</th>
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Total dû</th>
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Payé</th>
                            <th class="px-6 py-3.5 text-right text-sm font-medium text-on-surface-faint">Reste</th>
                            ...
                            <tr class="hover:bg-surface-hover/50 transition">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-medium text-on-surface">{{ $monthly->lease->tenant->full_name ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary">{{ $monthly->lease->property->reference ?? '-' }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary text-right">{{ number_format($monthly->rent_due, 0, ',', ' ') }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm text-on-surface-secondary text-right">{{ number_format($monthly->penalty_due, 0, ',', ' ') }}</td>
                                <td class="px-6 py-5 whitespace-nowrap text-sm font-semibold text-on-surface text-right">{{ number_format($monthly->total_due, 0, ',', ' ') }}</td>
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                    <a href="{{ route('monthlies.show', $monthly) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-on-surface-secondary bg-surface-hover border border-theme rounded-lg hover:bg-surface-alt transition"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>Voir</a>
                                    @if($monthly->status !== 'paye')
                                        @can('create', App\Models\Payment::class)
                                        <button type="button" data-open-modal="pay-monthly-{{ $monthly->id }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-accent-green-500 rounded-lg hover:bg-accent-green-600 transition shadow-sm"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg>Payer</button>
                                        @endcan
                                    @endif
                                    @if($monthly->status === 'paye')
                                        ...
                                        <form method="POST" action="{{ route('documents.generate-quittance') }}" class="inline">
                                            ...
                                            <button type="submit" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/30 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-800/40 dark:bg-blue-900/40 transition"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Quittance</button>
                                        </form>
                                        @endcan
                                    @endif
                                </td>
